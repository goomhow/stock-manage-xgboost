...
def get_transformed_data(month, frac=0.3):
    ...
    if not os.path.exists(dpath):
        fname = 'train_%s.csv' % month
        lname = 'label_%s.csv' % month
...
def trainFullAndPartFeature():
    ...
    joblib.dump(model_a, 'ADanCdmaModel_{}.pkl'.format(format(datetime.now().strftime('%d%H%M'))))
    X_train.drop(columns=rm_col, inplace=True)
    model_b.fit(X_train, y_train)
    joblib.dump(model_b, 'BDanCdmaModel_{}.pkl'.format(format(datetime.now().strftime('%d%H%M'))))
    del X_train
    del y_train
